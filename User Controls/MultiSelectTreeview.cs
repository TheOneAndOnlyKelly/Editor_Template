...
/// </summary>
public class DEL_MultiSelectTreeview : TreeView
{
	#region [ Constants ]
	...
	public static TreeNode FindByPath(TreeView treeControl, string fullPath)
	{
		...
		System.Windows.Forms.TreeNode Match = null;
		Name = NameList[0];

		foreach (System.Windows.Forms.TreeNode Node in treeControl.Nodes)
		{
			if (Node.Text == Name)
			{
				Match = Node;
		...
		for (int i = 1; i < NameList.Length; i++)
		{
			...
			foreach (System.Windows.Forms.TreeNode Node in Match.Nodes)
			{
				if (Node.Text == Name)
				{
					Match = Node;
